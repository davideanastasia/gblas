...
int main(int argc, char *argv[])
{
  ...
  cout << "TEST THROUGHPUT SGEMM" << endl << endl;
  ...
  const float MAX_VALUE = 128;
  const double VAR_A = 4.0*double(128*128)/12.0;
  const double VAR_B = VAR_A;     // to be changed if the inputs don't have the same features!
  ...
  float *A        = (float*)_mm_malloc(sizeof(float)*end_NB*end_NB, 16);
  float *B        = (float*)_mm_malloc(sizeof(float)*end_NB*end_NB, 16);
  float *C        = (float*)_mm_malloc(sizeof(float)*end_NB*end_NB, 16);
  float *C_final  = (float*)_mm_malloc(sizeof(float)*end_NB*end_NB, 16); 
  ...
  for (int NB = init_NB; NB <= end_NB; NB += GBLAS_KERNEL_SIZE)
  {
    ...
    N = NB; //+ 2*GBLAS_KERNEL_SIZE;
    M = NB; //+ GBLAS_KERNEL_SIZE;
    K = NB;
    
    double scale_factor = 2*K; //2*K*M*N;
    double peak_performance_s = 2660*8;
    ...
    for (int n_r = 0; n_r < NUM_RUNS; n_r++)
    {
      ...
      gblas_sgemm_plain(CblasRowMajor, CblasNoTrans, CblasNoTrans, M, N, K, alpha, A /* M x K */, K, B /* K x N */, N, beta, C_final /* M x N*/, N);
    ...
    Gflops = convert_to_gigaflops(bm_format_w_kernel_t.get_time()/NUM_RUNS, scale_factor)*M*N;
    cout << ", " << Gflops;
    cout << ", " << Gflops/peak_performance_s*100.0;
    
    cout << ", " << atlas_t.get_time()/NUM_RUNS;
    Gflops = convert_to_gigaflops(atlas_t.get_time()/NUM_RUNS, scale_factor)*M*N;
    cout << ", " << Gflops;
    cout << ", " << Gflops/peak_performance_s*100.0;
    cout << flush;
    ...
    for (int snr_p = 0; snr_p < NUM_SNR_TEST_POINT; snr_p ++)
    {
      ...
      for (int n_r = 0; n_r < NUM_RUNS; n_r++)
      {
        ...
        block_accerated = gblas_sgemm_snr(CblasRowMajor, CblasNoTrans, CblasNoTrans, M, N, K, alpha, A /* M x K */, K, B /* K x N */, N, beta, C /* M x N*/, N, SNR_TEST_POINTS[snr_p], VAR_A, VAR_B);
      ...
      Gflops = convert_to_gigaflops(bm_format_w_kernel_w_packing_t.get_time()/NUM_RUNS, scale_factor)*M*N;
      cout << ", " << Gflops;
      cout << ", " << Gflops/peak_performance_s*100.0;
      cout << flush;
    }
